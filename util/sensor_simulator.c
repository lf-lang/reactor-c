...
/** Table of Lingua Franca trigger objects to schedule in response to
 * keypresses. */
...
void _lf_show_message(const char* message_lines[], int number_of_lines) {
  ...
  getmaxyx(stdscr, term_height,
           term_width); // Get the size of the terminal window.
...
void _lf_start_tick_window(int width) {
  ...
  getmaxyx(stdscr, term_height,
           term_width); // Get the size of the terminal window.
  _lf_sensor.tick_window =
      newwin(term_height, width + 2, 0, term_width - width - 2);
...
void _lf_start_print_window(int above, int right) {
  ...
  _lf_sensor.print_window =
      newwin(_lf_sensor.print_window_height, term_width - right, above, 0);
...
void* _lf_sensor_read_input(void* ignored) {
  while (_lf_sensor.thread_created != 0) {
    ...
    if (c == EOF) {
      ...
      // because a _lf_sensor_trigger_table entry, once assigned a value,
      // becomes immutable.
      if (c == '\n' && _lf_sensor_sensor_newline_trigger != NULL) {
        lf_schedule_copy(_lf_sensor_sensor_newline_trigger, 0, &c, 1);
      } else if (c - 32 >= 0 && c - 32 < LF_SENSOR_TRIGGER_TABLE_SIZE &&
                 _lf_sensor_trigger_table[c - 32] != NULL) {
...
void* _lf_sensor_simulator_thread(void* ignored) {
  ...
  if (_lf_sensor.welcome_message != NULL &&
      _lf_sensor.welcome_message_length > 0) {
    _lf_show_message(_lf_sensor.welcome_message,
                     _lf_sensor.welcome_message_length);
  }
  _lf_sensor.tick_window = stdscr;
  if (_lf_sensor.tick_window_width > 0) {
    _lf_start_tick_window(_lf_sensor.tick_window_width);
  }
  _lf_start_print_window(_lf_sensor.welcome_message_length + 2,
                         _lf_sensor.tick_window_width + 2);
  ...
  // Here, we create a thread that produces no output and just listens for
  // input. This thread is exclusively responsible for producing output.
  int result = lf_thread_create(&_lf_sensor.input_thread_id,
                                &_lf_sensor_read_input, NULL);
  if (result != 0) {
    lf_print_error("Failed to start sensor simulator input listener!");
  }

  while (_lf_sensor.thread_created != 0) {
    // Sadly, ncurses is not thread safe, so this thread deals with all
    // messages.
    ...
    while (_lf_sensor.message_q != NULL) {
      if (_lf_sensor.message_q->type == _lf_sensor_close_windows) {
        ...
        wmove(_lf_sensor.tick_window, _lf_sensor.tick_cursor_y,
              _lf_sensor.tick_cursor_x);
        ...
        wmove(_lf_sensor.print_window, _lf_sensor.print_cursor_y,
              _lf_sensor.print_cursor_x);
...
int start_sensor_simulator(const char* message_lines[], int number_of_lines,
  ...
  if (_lf_sensor.thread_created == 0) {
    ...
    if (atexit(end_sensor_simulator) != 0) {
      lf_print_warning("sensor_simulator: Failed to register "
                       "end_sensor_simulator function!");
    }
    ...
    result = lf_thread_create(&_lf_sensor.output_thread_id,
                              &_lf_sensor_simulator_thread, NULL);
    if (result != 0) {
...
int register_sensor_key(char key, void* action) {
  ...
  int index = key - 32;
  if (key != '\n' && key != '\0' &&
      (index < 0 || index >= LF_SENSOR_TRIGGER_TABLE_SIZE)) {
    return 2;
  }
  int result = 0;
  lf_mutex_lock(&_lf_sensor_mutex);
