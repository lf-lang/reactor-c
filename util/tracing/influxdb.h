...
/*
  ...
  **NOTICE**: For best performance you should sort tags by key before sending
  them to the database. The sort should match the results from the [Go
  bytes.Compare function](https://golang.org/pkg/bytes/#Compare).
 */
...
int _escaped_append(char** dest, size_t* len, size_t* used, const char* src,
...
int post_http_send_line(influx_client_t* c, char* buf, int len) {
  ...
  for (;;) {
    iv[0].iov_len = snprintf((char*)iv[0].iov_base, len,
                             ...
                             c->db, c->usr ? c->usr : "", c->pwd ? c->pwd : "",
                             c->host, c->token ? c->token : "", iv[1].iov_len);
  ...
  memcpy(&addr.sin_addr, resolved_host->h_addr_list[0],
...
#define _GET_NEXT_CHAR()                                                       \
  (ch = (len >= (int)iv[0].iov_len &&                                          \
                 (iv[0].iov_len = recv(sock, iv[0].iov_base, iv[0].iov_len,    \
                                       len = 0)) == (size_t)(-1)               \
             ? 0                                                               \
...
int post_curl(influx_v2_client_t* c, ...) {
  ...
  snprintf(url_string, len,
           ...
           c->host ? c->host : "localhost", c->port ? c->port : 8086, c->org,
           c->bucket, c->precision ? c->precision : "ns");
  ...
  while (type != IF_TYPE_ARG_END) {
    if (type >= IF_TYPE_TAG && type <= IF_TYPE_FIELD_BOOLEAN) {
      if (last_type < IF_TYPE_MEAS ||
          last_type >
              (type == IF_TYPE_TAG ? IF_TYPE_TAG : IF_TYPE_FIELD_BOOLEAN))
        goto FAIL;
      _APPEND("%c",
