...
lf_waveform_t* read_wave_file(const char* path) {
  ...
  uint32_t expected_chunk_id =
      (uint32_t)'FFIR';                        // Little-endian version of RIFF.
  uint32_t expected_format = (uint32_t)'EVAW'; // Little-endian version of WAVE.
  uint32_t expected_subchunk_id =
      (uint32_t)' tmf'; // Little-endian version of 'fmt '.
  if (*(uint32_t*)wav.riff.chunk_id != expected_chunk_id ||
      ...
      *(uint32_t*)fmt.subchunk_id != expected_subchunk_id ||
      fmt.subchunk_size != 16 || fmt.audio_format != 1 ||
    ...
    fprintf(stderr, "Chunk ID was expected to be 'RIFF'. Got: '%c%c%c%c'.\n",
            wav.riff.chunk_id[0], wav.riff.chunk_id[1], wav.riff.chunk_id[2],
            wav.riff.chunk_id[3]);
    fprintf(stderr, "Format was expected to be 'WAVE'. Got: '%c%c%c%c'.\n",
            wav.riff.format[0], wav.riff.format[1], wav.riff.format[2],
            wav.riff.format[3]);
    fprintf(stderr, "Subchunk ID was expected to be 'fmt '. Got: '%c%c%c%c'.\n",
            fmt.subchunk_id[0], fmt.subchunk_id[1], fmt.subchunk_id[2],
            fmt.subchunk_id[3]);
    fprintf(stderr, "Subchunk size was expected to be 16. Got: '%d'.\n",
            fmt.subchunk_size);
    fprintf(stderr,
            "Audio format was expected to be 1 (LPCM, no compression). Got: "
            "'%d'.\n",
            fmt.audio_format);
  ...
  uint32_t expected_data_id =
      (uint32_t)'atad'; // Little-endian version of 'data'.
  while (*(uint32_t*)data.subchunk_id != expected_data_id) {
    ...
    if (bytes_read != data.subchunk_size) {
      fprintf(
          stderr,
          "Intermediate junk chunk '%c%c%c%c' could not be read. Giving up.\n",
          data.subchunk_id[0], data.subchunk_id[1], data.subchunk_id[2],
          data.subchunk_id[3]);
      break;
    }
  ...
  result->length =
      data.subchunk_size /
      2; // Subchunk size is in bytes, but length is number of samples.
  result->num_channels = num_channels;
  result->waveform = (int16_t*)calloc(data.subchunk_size / 2, sizeof(int16_t));

  size_t bytes_read =
      fread(result->waveform, sizeof(int16_t), data.subchunk_size / 2, fp);
