...
void test_put(hashmap_object2int_t* h) {
  ...
  int value = rand();
  hashmap_object2int_entry_t entry =
      (hashmap_object2int_entry_t){.key = key, .value = value};
...
void test_get(hashmap_object2int_t* h) {
  ...
  if (desired.value != found) {
    // It is possible that two distinct values were associated with the same
    // key. Search the "mock" array to check if this is the case.
