...
/**
 * Raise a barrier to prevent the current tag for the specified environment from
 * advancing to or beyond the value of the future_tag argument, if possible. If
 * the current tag is already at or beyond future_tag, then prevent any further
 * advances. This function will increment the total number of pending barrier
 * requests. For each call to this function, there should always be a subsequent
 ...
 * current tag, this function will raise a barrier to the current tag plus one
 * microstep.
 *
...
/**
 * @brief Version of _lf_increment_tag_barrier to call when the caller holds the
 * mutex. This version does not acquire the mutex belonging to env.
 *
 * @param env Environment within which we are executing.
 * @param future_tag A desired tag for the barrier. This function will guarantee
 * that current logical time will not go past future_tag if it is in the future.
...
/**
 * Decrement the total number of pending barrier requests for the environment
 * tag barrier. If the total number of requests reaches zero, this function
 * resets the tag barrier to FOREVER_TAG and notifies all threads that are
 * waiting on the barrier that the number of requests has reached zero.
 *
...
bool wait_until(environment_t* env, instant_t logical_time_ns,
                lf_cond_t* condition);
