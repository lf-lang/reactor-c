 ...
 *
 * Hashmaps are defined by redefining K, V, HASH_OF, and HASHMAP, and including
 * this file. A default hashmap type is defined here. See pointer_hashmap.h for
 * an example of a hashmap declaration.
 * - K and V must be the types of keys and values of the hashmap, respectively.
 * - HASH_OF must be the hash of a key.
 * - HASHMAP must be a function-like macro that prefixes tokens with the name of
 * the hashmap. For example, the name of the hashmap data type is given by
 * evaluation of the macro HASHMAP(t) so that it is "t" prefixed with the name
 * of the hashmap. The function names associated with the data type are similar.
 */
 ...
 * @param capacity A number that is much larger than the maximum number of items
 * that this hashmap will contain. Insufficient surplus capacity will cause poor
 * performance.
...
static HASHMAP(entry_t) *
    HASHMAP(get_ideal_address)(HASHMAP(t) * hashmap, K key) {
  HASHMAP(entry_t)* address =
      hashmap->entries + (HASH_OF(key) % hashmap->capacity);
...
/**
 * @brief Return the actual address of the hashmap entry corresponding to `key`,
 ...
 */
static HASHMAP(entry_t) *
    HASHMAP(get_actual_address)(HASHMAP(t) * hashmap, K key) {
...
HASHMAP(t) * HASHMAP(new)(size_t capacity, K nothing) {
  HASHMAP(entry_t)* entries =
      (HASHMAP(entry_t)*)malloc((capacity + 1) * sizeof(HASHMAP(entry_t)));
  if (!entries)
    exit(1);
  HASHMAP(t)* ret = (HASHMAP(t)*)malloc(sizeof(HASHMAP(t)));
  if (!ret)
    exit(1);
  // The entry at the end is used as a boundary. It will never again be written
  // to.
  ...
  ret->num_entries = 0;
  // A second nothing may be required if removal is to be supported and we want
  // to make removal a constant-time operation.
  ret->nothing = nothing;
  return ret;
}
