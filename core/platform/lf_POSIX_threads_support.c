...
int lf_cond_wait(lf_cond_t* cond) {
  return pthread_cond_wait((pthread_cond_t*)&cond->condition,
...
int lf_cond_timedwait(lf_cond_t* cond, int64_t absolute_time_ns) {
  ...
  struct timespec timespec_absolute_time = {
      (time_t)absolute_time_ns / 1000000000LL,
      (long)absolute_time_ns % 1000000000LL};
  int return_value = 0;
  return_value = pthread_cond_timedwait((pthread_cond_t*)&cond->condition,
                                        (pthread_mutex_t*)cond->mutex,
