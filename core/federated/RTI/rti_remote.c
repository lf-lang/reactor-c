...
void handle_port_absent_message(federate_info_t* sending_federate, unsigned char* buffer) {
  ...
  read_from_netchan_fail_on_error(sending_federate->fed_netchan, message_size, &(buffer[1]),
...
void handle_timed_message(federate_info_t* sending_federate, unsigned char* buffer) {
  ...
  read_from_netchan_fail_on_error(sending_federate->fed_netchan, header_size - 1, &(buffer[1]),
  ...
  read_from_netchan_fail_on_error(sending_federate->fed_netchan, bytes_to_read, &(buffer[header_size]),
  ...
  if (fed->enclave.state == NOT_CONNECTED) {
    ...
    while (total_bytes_read < total_bytes_to_read) {
      ...
      read_from_netchan_fail_on_error(sending_federate->fed_netchan, bytes_to_read, buffer,
...
void handle_latest_tag_confirmed(federate_info_t* fed) {
  ...
  read_from_netchan_fail_on_error(fed->fed_netchan, sizeof(int64_t) + sizeof(uint32_t), buffer,
...
void handle_stop_request_message(federate_info_t* fed) {
  ...
  read_from_netchan_fail_on_error(fed->fed_netchan, bytes_to_read, buffer,
...
void handle_stop_request_reply(federate_info_t* fed) {
  ...
  read_from_netchan_fail_on_error(fed->fed_netchan, bytes_to_read, buffer_stop_time,
...
void handle_address_query(uint16_t fed_id) {
  ...
  read_from_netchan_fail_on_error(fed->fed_netchan, sizeof(uint16_t), (unsigned char*)buffer,
...
void handle_address_ad(uint16_t federate_id) {
  ...
  read_from_netchan_fail_on_error(fed->fed_netchan, sizeof(int32_t), (unsigned char*)buffer,
...
void handle_timestamp(federate_info_t* my_fed) {
  ...
  read_from_netchan_fail_on_error(my_fed->fed_netchan, sizeof(int64_t), (unsigned char*)&buffer,
...
static int receive_connection_information(netchan_t fed_netchan, uint16_t fed_id) {
  ...
  read_from_netchan_fail_on_error(fed_netchan, MSG_TYPE_NEIGHBOR_STRUCTURE_HEADER_SIZE, connection_info_header,
  ...
  if (connection_info_header[0] != MSG_TYPE_NEIGHBOR_STRUCTURE) {
    ...
    if (connections_info_body_size > 0) {
      ...
      read_from_netchan_fail_on_error(fed_netchan, connections_info_body_size, connections_info_body,
...
static int receive_udp_message_and_set_up_clock_sync(netchan_t fed_netchan, uint16_t fed_id) {
  ...
  read_from_netchan_fail_on_error(fed_netchan, 1 + sizeof(uint16_t), response,
    ...
    if (rti_remote->clock_sync_global_status >= clock_sync_init) {
      ...
      if (federate_UDP_port_number != UINT16_MAX) {
        ...
        for (int i = 0; i < rti_remote->clock_sync_exchanges_per_interval; i++) {
          ...
          size_t message_size = 1 + sizeof(uint16_t);
          unsigned char buffer[message_size];
          read_from_netchan_fail_on_error(fed_netchan, message_size, buffer,
...
static bool authenticate_federate(netchan_t fed_netchan) {
  ...
  read_from_netchan_fail_on_error(fed_netchan, 1 + fed_id_length + NONCE_LENGTH, buffer,
  ...
  read_from_netchan_fail_on_error(fed_netchan, 1 + hmac_length, received, "Failed to read federate response.");
