...
// and casting it.
#define GET_FED_INFO(_idx)                                                     \
  (federate_info_t*)rti_remote->base.scheduling_nodes[_idx]
...
int create_server(int32_t specified_port, uint16_t port,
                  socket_type_t socket_type) {
  // Timeout time for the communications of the server
  struct timeval timeout_time = {.tv_sec = TCP_TIMEOUT_TIME / BILLION,
                                 .tv_usec =
                                     (TCP_TIMEOUT_TIME % BILLION) / 1000};
  ...
  if (socket_type == TCP) {
    ...
    timeout_time =
        (struct timeval){.tv_sec = UDP_TIMEOUT_TIME / BILLION,
  ...
  int true_variable =
      1; // setsockopt() requires a reference to the value assigned to an option
  if (setsockopt(socket_descriptor, SOL_SOCKET, SO_REUSEADDR, &true_variable,
                 sizeof(int32_t)) < 0) {
    lf_print_error("RTI failed to set SO_REUSEADDR option on the socket: %s.",
                   strerror(errno));
  }
  // Set the timeout on the socket so that read and write operations don't block
  // for too long
  if (setsockopt(socket_descriptor, SOL_SOCKET, SO_RCVTIMEO,
                 (const char*)&timeout_time, sizeof(timeout_time)) < 0) {
    lf_print_error("RTI failed to set SO_RCVTIMEO option on the socket: %s.",
                   strerror(errno));
  }
  if (setsockopt(socket_descriptor, SOL_SOCKET, SO_SNDTIMEO,
                 (const char*)&timeout_time, sizeof(timeout_time)) < 0) {
    lf_print_error("RTI failed to set SO_SNDTIMEO option on the socket: %s.",
                   strerror(errno));
  }
  ...
  */
  ...
  lf_print("RTI using %s port %d for federation %s.", type, port,
...
void notify_tag_advance_grant(scheduling_node_t* e, tag_t tag) {
  ...
  // function. During this call, the socket might close, causing the following
  // write_to_socket to fail. Consider a failure here a soft failure and update
  // the federate's status.
  ssize_t bytes_written =
      write_to_socket(((federate_info_t*)e)->socket, message_length, buffer);
  if (bytes_written < (ssize_t)message_length) {
    lf_print_error("RTI failed to send tag advance grant to federate %d.",
                   e->id);
    if (bytes_written < 0) {
      e->state = NOT_CONNECTED;
      // FIXME: We need better error handling, but don't stop other execution
      // here.
    }
  } else {
    e->last_granted = tag;
    LF_PRINT_LOG(
        "RTI sent to federate %d the tag advance grant (TAG) " PRINTF_TAG ".",
...
void notify_provisional_tag_advance_grant(scheduling_node_t* e, tag_t tag) {
  ...
  if (bytes_written < (ssize_t)message_length) {
    ...
    LF_PRINT_LOG("RTI sent to federate %d the Provisional Tag Advance Grant "
                 "(PTAG) " PRINTF_TAG ".",
                 e->id, tag.time - start_time, tag.microstep);
...
void update_federate_next_event_tag_locked(uint16_t federate_id,
  ...
  tag_t min_in_transit_tag =
      get_minimum_in_transit_message_tag(fed->in_transit_message_tags);
...
void handle_port_absent_message(federate_info_t* sending_federate,
  ...
  read_from_socket_errexit(
      sending_federate->socket, message_size, &(buffer[1]),
  ...
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_PORT_ABS,
                                 sending_federate->enclave.id, &tag);
  }
  ...
  if (fed->enclave.state == NOT_CONNECTED) {
    ...
    lf_print_warning("RTI: Destination federate %d is no longer connected. "
                     "Dropping message.",
                     federate_id);
  ...
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_to_federate(rti_remote->base.trace, send_PORT_ABS,
                               federate_id, &tag);
  }
  write_to_socket_errexit(destination_socket, message_size + 1, buffer,
                          "RTI failed to forward message to federate %d.",
                          federate_id);
...
void handle_timed_message(federate_info_t* sending_federate,
                          unsigned char* buffer) {
  size_t header_size = 1 + sizeof(uint16_t) + sizeof(uint16_t) +
                       sizeof(int32_t) + sizeof(int64_t) + sizeof(uint32_t);
  // Read the header, minus the first byte which has already been read.
  read_from_socket_errexit(
      sending_federate->socket, header_size - 1, &(buffer[1]),
  ...
  extract_timed_header(&(buffer[1]), &reactor_port_id, &federate_id, &length,
                       &intended_tag);
  ...
  LF_PRINT_LOG("RTI received message from federate %d for federate %u port %u "
               "with intended tag " PRINTF_TAG ". Forwarding.",
               sending_federate->enclave.id, federate_id, reactor_port_id,
               intended_tag.time - lf_time_start(), intended_tag.microstep);

  read_from_socket_errexit(
      sending_federate->socket, bytes_to_read, &(buffer[header_size]),
  ...
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_TAGGED_MSG,
                                 sending_federate->enclave.id, &intended_tag);
  }
  ...
  if (lf_tag_compare(fed->enclave.completed, intended_tag) < 0) {
    // Add a record of this message to the list of in-transit messages to this
    // federate.
    add_in_transit_message_record(fed->in_transit_message_tags, intended_tag);
    LF_PRINT_DEBUG("RTI: Adding a message with tag " PRINTF_TAG
                   " to the list of in-transit messages for federate %d.",
                   intended_tag.time - lf_time_start(), intended_tag.microstep,
                   federate_id);
  } else {
    lf_print_error("RTI: Federate %d has already completed tag " PRINTF_TAG
                   ...
                   " from federate %hu. "
                   "This is going to cause an STP violation under centralized "
                   "coordination.",
                   federate_id, fed->enclave.completed.time - lf_time_start(),
                   fed->enclave.completed.microstep,
                   intended_tag.time - lf_time_start(), intended_tag.microstep,
                   sending_federate->enclave.id);
  ...
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_to_federate(rti_remote->base.trace, send_TAGGED_MSG,
                               federate_id, &intended_tag);
  }
...
void handle_logical_tag_complete(federate_info_t* fed) {
  ...
  read_from_socket_errexit(fed->socket, sizeof(int64_t) + sizeof(uint32_t),
                           buffer,
                           "RTI failed to read the content of the logical tag "
                           "complete from federate %d.",
                           fed->enclave.id);
  tag_t completed = extract_tag(buffer);
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_LTC,
                                 fed->enclave.id, &completed);
  }
  ...
  clean_in_transit_message_record_up_to_tag(fed->in_transit_message_tags,
...
void handle_next_event_tag(federate_info_t* fed) {
  unsigned char buffer[sizeof(int64_t) + sizeof(uint32_t)];
  read_from_socket_errexit(
      fed->socket, sizeof(int64_t) + sizeof(uint32_t), buffer,
  ...
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_NET,
                                 fed->enclave.id, &intended_tag);
  }
  LF_PRINT_LOG(
      "RTI received from federate %d the Next Event Tag (NET) " PRINTF_TAG,
      fed->enclave.id, intended_tag.time - start_time, intended_tag.microstep);
...
void _lf_rti_broadcast_stop_time_to_federates_locked() {
  ...
  ENCODE_STOP_GRANTED(outgoing_buffer, rti_remote->base.max_stop_tag.time,
  ...
  for (int i = 0; i < rti_remote->base.number_of_scheduling_nodes; i++) {
    ...
    if (lf_tag_compare(fed->enclave.next_event,
                       rti_remote->base.max_stop_tag) >= 0) {
      // Need the next_event to be no greater than the stop tag.
      fed->enclave.next_event = rti_remote->base.max_stop_tag;
    }
    if (rti_remote->base.tracing_enabled) {
      tracepoint_rti_to_federate(rti_remote->base.trace, send_STOP_GRN,
                                 fed->enclave.id,
                                 &rti_remote->base.max_stop_tag);
    }
    write_to_socket_errexit(
        fed->socket, MSG_TYPE_STOP_GRANTED_LENGTH, outgoing_buffer,
        "RTI failed to send MSG_TYPE_STOP_GRANTED message to federate %d.",
        fed->enclave.id);
  }

  LF_PRINT_LOG(
      "RTI sent to federates MSG_TYPE_STOP_GRANTED with tag (" PRINTF_TIME
      ", %u).",
...
void handle_stop_request_message(federate_info_t* fed) {
  LF_PRINT_DEBUG("RTI handling stop_request from federate %d.",
                 fed->enclave.id);

  size_t bytes_to_read = MSG_TYPE_STOP_REQUEST_LENGTH - 1;
  unsigned char buffer[bytes_to_read];
  read_from_socket_errexit(
      fed->socket, bytes_to_read, buffer,
  ...
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_STOP_REQ,
  ...
  LF_PRINT_LOG("RTI received from federate %d a MSG_TYPE_STOP_REQUEST message "
               "with tag " PRINTF_TAG ".",
               fed->enclave.id, proposed_stop_tag.time - start_time,
  ...
  ENCODE_STOP_REQUEST(stop_request_buffer, rti_remote->base.max_stop_tag.time,
                      rti_remote->base.max_stop_tag.microstep);

  // Iterate over federates and send each the MSG_TYPE_STOP_REQUEST message
  // if we do not have a stop_time already for them. Do not do this more than
  // once.
  ...
  for (int i = 0; i < rti_remote->base.number_of_scheduling_nodes; i++) {
    ...
    if (f->enclave.id != fed->enclave.id && f->requested_stop == false) {
      ...
      if (rti_remote->base.tracing_enabled) {
        tracepoint_rti_to_federate(rti_remote->base.trace, send_STOP_REQ,
                                   f->enclave.id,
                                   &rti_remote->base.max_stop_tag);
      }
      write_to_socket_errexit(
          f->socket, MSG_TYPE_STOP_REQUEST_LENGTH, stop_request_buffer,
  ...
  LF_PRINT_LOG(
      "RTI forwarded to federates MSG_TYPE_STOP_REQUEST with tag (" PRINTF_TIME
      ", %u).",
      rti_remote->base.max_stop_tag.time - start_time,
      rti_remote->base.max_stop_tag.microstep);
  lf_mutex_unlock(&rti_mutex);
...
void handle_stop_request_reply(federate_info_t* fed) {
  ...
  read_from_socket_errexit(fed->socket, bytes_to_read, buffer_stop_time,
                           "RTI failed to read the reply to "
                           "MSG_TYPE_STOP_REQUEST message from federate %d.",
                           fed->enclave.id);

  tag_t federate_stop_tag = extract_tag(buffer_stop_time);

  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_STOP_REQ_REP,
                                 fed->enclave.id, &federate_stop_tag);
  }

  LF_PRINT_LOG("RTI received from federate %d STOP reply tag " PRINTF_TAG ".",
               fed->enclave.id, federate_stop_tag.time - start_time,
...
void handle_address_query(uint16_t fed_id) {
  ...
  ssize_t bytes_read =
      read_from_socket(fed->socket, sizeof(uint16_t), (unsigned char*)buffer);
  if (bytes_read == 0) {
  ...
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_ADR_QR, fed_id,
                                 NULL);
  }

  LF_PRINT_DEBUG("RTI received address query from %d for %d.", fed_id,
                 remote_fed_id);

  // NOTE: server_port initializes to -1, which means the RTI does not know
  // the port number because it has not yet received an
  // MSG_TYPE_ADDRESS_ADVERTISEMENT message from this federate. In that case, it
  // will respond by sending -1.
  ...
  write_to_socket_errexit(
      fed->socket, sizeof(int32_t), (unsigned char*)buffer,
  ...
  if (remote_fed->server_port != -1) {
    LF_PRINT_DEBUG(
        "Replied to address query from federate %d with address %s:%d.", fed_id,
        remote_fed->server_hostname, remote_fed->server_port);
  }
}
...
void handle_address_ad(uint16_t federate_id) {
  ...
  unsigned char buffer[sizeof(int32_t)];
  ssize_t bytes_read =
      read_from_socket(fed->socket, sizeof(int32_t), (unsigned char*)buffer);
  ...
  if (rti_remote->base.tracing_enabled) {
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_ADR_AD,
                                 federate_id, NULL);
  }
...
void handle_timestamp(federate_info_t* my_fed) {
  ...
  ssize_t bytes_read = read_from_socket(my_fed->socket, sizeof(int64_t),
                                        (unsigned char*)&buffer);
  ...
  if (rti_remote->base.tracing_enabled) {
    ...
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_TIMESTAMP,
                                 my_fed->enclave.id, &tag);
  }
  LF_PRINT_DEBUG("RTI received timestamp message with time: " PRINTF_TIME ".",
                 timestamp);
  ...
  encode_int64(swap_bytes_if_big_endian_int64(start_time),
               &start_time_buffer[1]);

  if (rti_remote->base.tracing_enabled) {
    tag_t tag = {.time = start_time, .microstep = 0};
    tracepoint_rti_to_federate(rti_remote->base.trace, send_TIMESTAMP,
                               my_fed->enclave.id, &tag);
  }
  ssize_t bytes_written = write_to_socket(
      my_fed->socket, MSG_TYPE_TIMESTAMP_LENGTH, start_time_buffer);
  if (bytes_written < MSG_TYPE_TIMESTAMP_LENGTH) {
    lf_print_error("Failed to send the starting time to federate %d.",
                   my_fed->enclave.id);
  }

  lf_mutex_lock(&rti_mutex);
  // Update state for the federate to indicate that the MSG_TYPE_TIMESTAMP
  // message has been sent. That MSG_TYPE_TIMESTAMP message grants time advance
  // to the federate to the start time.
  my_fed->enclave.state = GRANTED;
  lf_cond_broadcast(&sent_start_time);
  LF_PRINT_LOG("RTI sent start time " PRINTF_TIME " to federate %d.",
               start_time, my_fed->enclave.id);
  lf_mutex_unlock(&rti_mutex);
}
...
void send_physical_clock(unsigned char message_type, federate_info_t* fed,
                         socket_type_t socket_type) {
  if (fed->enclave.state == NOT_CONNECTED) {
    lf_print_warning("Clock sync: RTI failed to send physical time to federate "
                     "%d. Socket not connected.\n",
                     fed->enclave.id);
    return;
  }
  ...
  if (socket_type == UDP) {
    ...
    ssize_t bytes_written =
        sendto(rti_remote->socket_descriptor_UDP, buffer, 1 + sizeof(int64_t),
               0, (struct sockaddr*)&fed->UDP_addr, sizeof(fed->UDP_addr));
    if (bytes_written < (ssize_t)sizeof(int64_t) + 1) {
      lf_print_warning(
          "Clock sync: RTI failed to send physical time to federate %d: %s\n",
          fed->enclave.id, strerror(errno));
      return;
    }
  } else if (socket_type == TCP) {
    LF_PRINT_DEBUG("Clock sync:  RTI sending TCP message type %u.", buffer[0]);
    write_to_socket_errexit(
        fed->socket, 1 + sizeof(int64_t), buffer,
        "Clock sync: RTI failed to send physical time to federate %d: %s.",
        fed->enclave.id, strerror(errno));
  }
  LF_PRINT_DEBUG("Clock sync: RTI sent PHYSICAL_TIME_SYNC_MESSAGE with "
                 "timestamp " PRINTF_TIME " to federate %d.",
...
void* clock_synchronization_thread(void* noargs) {
  ...
  // Initiate a clock synchronization every rti->clock_sync_period_ns
  struct timespec sleep_time = {(time_t)rti_remote->clock_sync_period_ns /
                                    BILLION,
  ...
  while (any_federates_connected) {
    ...
    for (int fed_id = 0; fed_id < rti_remote->base.number_of_scheduling_nodes;
         fed_id++) {
      federate_info_t* fed = GET_FED_INFO(fed_id);
      if (fed->enclave.state == NOT_CONNECTED) {
        // FIXME: We need better error handling here, but clock sync failure
        // should not stop execution.
        lf_print_error("Clock sync failed with federate %d. Not connected.",
                       fed_id);
        continue;
      ...
      // Maximum number of messages that we discard before giving up on this
      // cycle. If the T3 message from this federate does not arrive and we keep
      // receiving other messages, then give up on this federate and move to the
      // next federate.
      int remaining_attempts = 5;
      while (remaining_attempts > 0) {
        remaining_attempts--;
        int bytes_read = read_from_socket(rti_remote->socket_descriptor_UDP,
                                          message_size, buffer);
        // If any errors occur, either discard the message or the clock sync
        // round.
        if (bytes_read == message_size) {
          if (buffer[0] == MSG_TYPE_CLOCK_SYNC_T3) {
            ...
            if (fed_id_2 != fed->enclave.id) {
              ...
              lf_print_warning(
                  "Clock sync: Received T3 message from federate %d, "
                  "but expected one from %d. Discarding message.",
                  fed_id_2, fed->enclave.id);
              continue;
            }
            LF_PRINT_DEBUG(
                "Clock sync: RTI received T3 message from federate %d.",
                fed_id_2);
            ...
            lf_print_warning("Clock sync: Unexpected UDP message %u. Expected "
                             "%u from federate %d. "
                             "Discarding message.",
                             buffer[0], MSG_TYPE_CLOCK_SYNC_T3,
                             fed->enclave.id);
            continue;
          }
        } else {
...
void handle_federate_resign(federate_info_t* my_fed) {
  ...
  if (rti_remote->base.tracing_enabled) {
    ...
    // Read the header, minus the first byte which has already been read.
    read_from_socket_errexit(
        my_fed->socket, header_size - 1, &(buffer[1]),
        "RTI failed to read the timed message header from remote federate.");
    // Extract the tag sent by the resigning federate
    tag_t tag = extract_tag(&(buffer[1]));
    tracepoint_rti_from_federate(rti_remote->base.trace, receive_RESIGN,
                                 my_fed->enclave.id, &tag);
  }
  ...
  // According to this:
  // https://stackoverflow.com/questions/4160347/close-vs-shutdown-socket, the
  // close should happen when receiving a 0 length message from the other end.
  // Here, we just signal the other side that no further writes to the socket
  // are forthcoming, which should result in the other end getting a zero-length
  // reception.
...
void* federate_info_thread_TCP(void* fed) {
  ...
  while (my_fed->enclave.state != NOT_CONNECTED) {
    ...
    if (bytes_read < 1) {
      // Socket is closed
      lf_print_warning(
          "RTI: Socket to federate %d is closed. Exiting the thread.",
          my_fed->enclave.id);
      my_fed->enclave.state = NOT_CONNECTED;
      my_fed->socket = -1;
      // FIXME: We need better error handling here, but do not stop execution
      // here.
      break;
    }
    LF_PRINT_DEBUG("RTI: Received message type %u from federate %d.", buffer[0],
                   my_fed->enclave.id);
    switch (buffer[0]) {
    ...
    case MSG_TYPE_STOP_REQUEST:
      handle_stop_request_message(
          my_fed); // FIXME: Reviewed until here.
    ...
    default:
      lf_print_error(
          "RTI received from federate %d an unrecognized TCP message type: %u.",
          my_fed->enclave.id, buffer[0]);
      if (rti_remote->base.tracing_enabled) {
        tracepoint_rti_from_federate(rti_remote->base.trace,
                                     receive_UNIDENTIFIED, my_fed->enclave.id,
                                     NULL);
      }
    }
  }
...
void send_reject(int socket_id, unsigned char error_code) {
  ...
  write_to_socket_errexit(
      socket_id, 2, response,
      "RTI failed to write MSG_TYPE_REJECT message on the socket.");
  // Close the socket.
  close(socket_id);
}

int32_t receive_and_check_fed_id_message(int socket_id,
                                         struct sockaddr_in* client_fd) {
  // Buffer for message ID, federate ID, and federation ID length.
  size_t length = 1 + sizeof(uint16_t) +
                  1; // Message ID, federate ID, length of fedration ID.
  unsigned char buffer[length];

  // Read bytes from the socket. We need 4 bytes.
  // FIXME: This should not exit with error but rather should just reject the
  // connection.
  read_from_socket_errexit(socket_id, length, buffer,
                           "RTI failed to read from accepted socket.");

  uint16_t fed_id =
      rti_remote->base
          .number_of_scheduling_nodes; // Initialize to an invalid value.

  // First byte received is the message type.
  if (buffer[0] != MSG_TYPE_FED_IDS) {
    if (buffer[0] == MSG_TYPE_P2P_SENDING_FED_ID ||
        buffer[0] == MSG_TYPE_P2P_TAGGED_MESSAGE) {
    ...
    if (rti_remote->base.tracing_enabled) {
      tracepoint_rti_to_federate(rti_remote->base.trace, send_REJECT, fed_id,
                                 NULL);
    }
    lf_print_error(
        "RTI expected a MSG_TYPE_FED_IDS message. Got %u (see net_common.h).",
        buffer[0]);
    return -1;
  } else {
    ...
    char federation_id_received[federation_id_length +
                                1]; // One extra for null terminator.
    // Next read the actual federation ID.
    // FIXME: This should not exit on error, but rather just reject the
    // connection.
    read_from_socket_errexit(
    ...
    if (rti_remote->base.tracing_enabled) {
      tracepoint_rti_from_federate(rti_remote->base.trace, receive_FED_ID,
                                   fed_id, NULL);
    }
    // Compare the received federation ID to mine.
    if (strncmp(rti_remote->federation_id, federation_id_received,
                federation_id_length) != 0) {
      // Federation IDs do not match. Send back a MSG_TYPE_REJECT message.
      lf_print_error("WARNING: Federate from another federation %s attempted "
                     "to connect to RTI in federation %s.\n",
      ...
      if (fed_id >= rti_remote->base.number_of_scheduling_nodes) {
        ...
        lf_print_error("RTI received federate ID %d, which is out of range.",
                       fed_id);
        if (rti_remote->base.tracing_enabled) {
          tracepoint_rti_to_federate(rti_remote->base.trace, send_REJECT,
                                     fed_id, NULL);
        }
        send_reject(socket_id, FEDERATE_ID_OUT_OF_RANGE);
        return -1;
      } else {
        if ((rti_remote->base.scheduling_nodes[fed_id])->state !=
            NOT_CONNECTED) {
  ...
  // The IP address is stored here as an in_addr struct (in .server_ip_addr)
  // that can be useful to create sockets and can be efficiently sent over the
  // network. First, convert the sockaddr structure into a sockaddr_in that
  // contains an internet address.
  struct sockaddr_in* pV4_addr = client_fd;
  // Then extract the internet address (which is in IPv4 format) and assign it
  // as the federate's socket server
  ...
  LF_PRINT_DEBUG("RTI got address %s from federate %d.", fed->server_hostname,
                 fed_id);
#endif
...
int receive_connection_information(int socket_id, uint16_t fed_id) {
  LF_PRINT_DEBUG(
      "RTI waiting for MSG_TYPE_NEIGHBOR_STRUCTURE from federate %d.", fed_id);
  unsigned char connection_info_header[MSG_TYPE_NEIGHBOR_STRUCTURE_HEADER_SIZE];
  read_from_socket_errexit(socket_id, MSG_TYPE_NEIGHBOR_STRUCTURE_HEADER_SIZE,
                           connection_info_header,
                           "RTI failed to read MSG_TYPE_NEIGHBOR_STRUCTURE "
                           "message header from federate %d.",
                           fed_id);

  if (connection_info_header[0] != MSG_TYPE_NEIGHBOR_STRUCTURE) {
    lf_print_error("RTI was expecting a MSG_TYPE_UDP_PORT message from "
                   "federate %d. Got %u instead. "
                   "Rejecting federate.",
    ...
    fed->enclave.num_downstream =
        extract_int32(&(connection_info_header[1 + sizeof(int32_t)]));
    LF_PRINT_DEBUG("RTI got %d upstreams and %d downstreams from federate %d.",
                   fed->enclave.num_upstream, fed->enclave.num_downstream,
                   fed_id);

    // Allocate memory for the upstream and downstream pointers
    fed->enclave.upstream =
        (int*)malloc(sizeof(uint16_t) * fed->enclave.num_upstream);
    fed->enclave.downstream =
        (int*)malloc(sizeof(uint16_t) * fed->enclave.num_downstream);

    // Allocate memory for the upstream delay pointers
    fed->enclave.upstream_delay =
        (interval_t*)malloc(sizeof(interval_t) * fed->enclave.num_upstream);
    ...
    unsigned char* connections_info_body =
        (unsigned char*)malloc(connections_info_body_size);
    read_from_socket_errexit(socket_id, connections_info_body_size,
                             connections_info_body,
                             "RTI failed to read MSG_TYPE_NEIGHBOR_STRUCTURE "
                             "message body from federate %d.",
                             fed_id);
    ...
    for (int i = 0; i < fed->enclave.num_upstream; i++) {
      fed->enclave.upstream[i] =
          extract_uint16(&(connections_info_body[message_head]));
      message_head += sizeof(uint16_t);
      fed->enclave.upstream_delay[i] =
          extract_int64(&(connections_info_body[message_head]));
      message_head += sizeof(int64_t);
    }

    // Next, read the info about downstream federates
    for (int i = 0; i < fed->enclave.num_downstream; i++) {
      fed->enclave.downstream[i] =
          extract_uint16(&(connections_info_body[message_head]));
      message_head += sizeof(uint16_t);
    }
...
int receive_udp_message_and_set_up_clock_sync(int socket_id, uint16_t fed_id) {
  // Read the MSG_TYPE_UDP_PORT message from the federate regardless of the
  // status of clock synchronization. This message will tell the RTI whether the
  // federate is doing clock synchronization, and if it is, what port to use for
  // UDP.
  LF_PRINT_DEBUG("RTI waiting for MSG_TYPE_UDP_PORT from federate %d.", fed_id);
  unsigned char response[1 + sizeof(uint16_t)];
  read_from_socket_errexit(
      socket_id, 1 + sizeof(uint16_t), response,
  ...
  if (response[0] != MSG_TYPE_UDP_PORT) {
    ...
    federate_info_t* fed = GET_FED_INFO(fed_id);
    if (rti_remote->clock_sync_global_status >=
        clock_sync_init) { // If no initial clock sync, no need perform initial
                           // clock sync.
      uint16_t federate_UDP_port_number = extract_uint16(&(response[1]));

      LF_PRINT_DEBUG("RTI got MSG_TYPE_UDP_PORT %u from federate %d.",
                     federate_UDP_port_number, fed_id);

      // A port number of UINT16_MAX means initial clock sync should not be
      // performed.
      if (federate_UDP_port_number != UINT16_MAX) {
        ...
        for (int i = 0; i < rti_remote->clock_sync_exchanges_per_interval;
             i++) {
          ...
          read_from_socket_errexit(socket_id, message_size, buffer,
                                   "Socket to federate %d unexpectedly closed.",
                                   fed_id);
          if (buffer[0] == MSG_TYPE_CLOCK_SYNC_T3) {
            ...
            assert(fed_id < 65536);
            LF_PRINT_DEBUG(
                "RTI received T3 clock sync message from federate %d.", fed_id);
            handle_physical_clock_sync_message(fed, TCP);
          } else {
            lf_print_error("Unexpected message %u from federate %d.", buffer[0],
                           fed_id);
            send_reject(socket_id, UNEXPECTED_MESSAGE);
            return 0;
          }
        ...
        LF_PRINT_DEBUG(
            "RTI finished initial clock synchronization with federate %d.",
            fed_id);
      }
      if (rti_remote->clock_sync_global_status >=
          clock_sync_on) { // If no runtime clock sync, no need to set up the
                           // UDP port.
      ...
      // Clock synchronization is universally disabled via the clock-sync
      // command-line parameter
      // (-c off was passed to the RTI).
      // Note that the federates are still going to send a MSG_TYPE_UDP_PORT
      // message but with a payload (port) of -1.
      fed->clock_synchronization_enabled = false;
    }
  }
  return 1;
...
bool authenticate_federate(int socket) {
  ...
  unsigned char* ret =
      HMAC(EVP_sha256(), rti_remote->federation_id, federation_id_length,
           mac_buf, 1 + fed_id_length + NONCE_LENGTH, hmac_tag, &hmac_length);
  if (ret == NULL) {
    lf_print_error_and_exit(
        "HMAC construction failed for MSG_TYPE_RTI_RESPONSE.");
  }
...
void connect_to_federates(int socket_descriptor) {
  for (int i = 0; i < rti_remote->base.number_of_scheduling_nodes; i++) {
    ...
    while (1) {
      socket_id =
          accept(rti_remote->socket_descriptor_TCP, &client_fd, &client_length);
      if (socket_id >= 0) {
        // Got a socket
        break;
      } else if (socket_id < 0 && (errno != EAGAIN || errno != EWOULDBLOCK)) {
        lf_print_error_and_exit("RTI failed to accept the socket. %s.",
                                strerror(errno));
      } else {
        // Try again
        lf_print_warning("RTI failed to accept the socket. %s. Trying again.",
                         strerror(errno));
        continue;
      }
    }
...
// Wait for the first message from the federate when RTI -a option is on.
    ...
    // The first message from the federate should contain its ID and the
    // federation ID.
    int32_t fed_id = receive_and_check_fed_id_message(
        socket_id, (struct sockaddr_in*)&client_fd);
    if (fed_id >= 0 &&
        receive_connection_information(socket_id, (uint16_t)fed_id) &&
        receive_udp_message_and_set_up_clock_sync(socket_id,
                                                  (uint16_t)fed_id)) {
  ...
  if (rti_remote->clock_sync_global_status >= clock_sync_on) {
    // Create the thread that performs periodic PTP clock synchronization
    // sessions over the UDP channel, but only if the UDP channel is open and at
    // least one federate is performing runtime clock synchronization.
    ...
    if (rti_remote->final_port_UDP != UINT16_MAX && clock_sync_enabled) {
      lf_thread_create(&rti_remote->clock_thread, clock_synchronization_thread,
                       NULL);
    }
  }
}
...
void* respond_to_erroneous_connections(void* nothing) {
  while (true) {
    ...
    lf_print_error("RTI received an unexpected connection request. Federation "
                   "is running.");
    unsigned char response[2];
    response[0] = MSG_TYPE_REJECT;
    ...
    write_to_socket_errexit(socket_id, 2, response,
                            "RTI failed to write FEDERATION_ID_DOES_NOT_MATCH "
                            "to erroneous incoming connection.");
    // Close the socket.
    close(socket_id);
  }
  return NULL;
...
int32_t start_rti_server(uint16_t port) {
  ...
  if (rti_remote->clock_sync_global_status >= clock_sync_on) {
    rti_remote->socket_descriptor_UDP =
        create_server(specified_port, rti_remote->final_port_TCP + 1, UDP);
  }
...
void wait_for_federates(int socket_descriptor) {
  ...
  // The socket server will not continue to accept connections after all the
  // federates have joined. In case some other federation's federates are trying
  // to join the wrong federation, need to respond. Start a separate thread to
  // do that.
  ...
  if (shutdown(socket_descriptor, SHUT_RDWR)) {
    LF_PRINT_LOG("On shut down TCP socket, received reply: %s",
                 strerror(errno));
  }
  ...
  if (rti_remote->socket_descriptor_UDP > 0) {
    if (shutdown(rti_remote->socket_descriptor_UDP, SHUT_RDWR)) {
      LF_PRINT_LOG("On shut down UDP socket, received reply: %s",
                   strerror(errno));
    }
...
void initialize_RTI(rti_remote_t* rti) {
  ...
  LF_ASSERT(lf_cond_init(&received_start_times, &rti_mutex) == 0,
            "Failed to initialize Condition Variable");
  LF_ASSERT(lf_cond_init(&sent_start_time, &rti_mutex) == 0,
            "Failed to initialize Condition Variable");
