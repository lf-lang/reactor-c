...
void notify_tag_advance_grant(scheduling_node_t* e, tag_t tag) {
  ...
  // function. During this call, the network abstraction might close, causing the following write_to_net_abstraction
  // to fail. Consider a failure here a soft failure and update the federate's status.
  if (write_to_net_abstraction(((federate_info_t*)e)->fed_net_abstraction, message_length, buffer)) {
...
void handle_port_absent_message(federate_info_t* sending_federate, unsigned char* buffer) {
  ...
  read_from_net_abstraction_fail_on_error(sending_federate->fed_net_abstraction, message_size, &(buffer[1]),
  ...
  // messages coming from the network abstraction connected to the destination does not
  ...
  // Forward the message.
  write_to_net_abstraction_fail_on_error(fed->fed_net_abstraction, message_size + 1, buffer, &rti_mutex,
...
void handle_timed_message(federate_info_t* sending_federate, unsigned char* buffer) {
  ...
  read_from_net_abstraction_fail_on_error(sending_federate->fed_net_abstraction, header_size - 1, &(buffer[1]),
  ...
  read_from_net_abstraction_fail_on_error(sending_federate->fed_net_abstraction, bytes_to_read, &(buffer[header_size]),
  ...
  if (fed->enclave.state == NOT_CONNECTED) {
    ...
    while (total_bytes_read < total_bytes_to_read) {
      ...
      read_from_net_abstraction_fail_on_error(sending_federate->fed_net_abstraction, bytes_to_read, buffer,
  ...
  write_to_net_abstraction_fail_on_error(fed->fed_net_abstraction, bytes_read, buffer, &rti_mutex,
                                         "RTI failed to forward message to federate %d.", federate_id);
  ...
  while (total_bytes_read < total_bytes_to_read) {
    ...
    write_to_net_abstraction_fail_on_error(fed->fed_net_abstraction, bytes_to_read, buffer, &rti_mutex,
...
void handle_latest_tag_confirmed(federate_info_t* fed) {
  ...
  read_from_net_abstraction_fail_on_error(
      fed->fed_net_abstraction, sizeof(int64_t) + sizeof(uint32_t), buffer,
...
void handle_next_event_tag(federate_info_t* fed) {
  unsigned char buffer[sizeof(int64_t) + sizeof(uint32_t)];
  read_from_net_abstraction_fail_on_error(fed->fed_net_abstraction, sizeof(int64_t) + sizeof(uint32_t), buffer,
...
static void broadcast_stop_time_to_federates_locked() {
  ...
  for (int i = 0; i < rti_remote->base.number_of_scheduling_nodes; i++) {
    ...
    write_to_net_abstraction_fail_on_error(
        fed->fed_net_abstraction, MSG_TYPE_STOP_GRANTED_LENGTH, outgoing_buffer, &rti_mutex,
...
void handle_stop_request_message(federate_info_t* fed) {
  ...
  read_from_net_abstraction_fail_on_error(fed->fed_net_abstraction, bytes_to_read, buffer,
  ...
  for (int i = 0; i < rti_remote->base.number_of_scheduling_nodes; i++) {
    ...
    if (f->enclave.id != fed->enclave.id && f->requested_stop == false) {
      ...
      write_to_net_abstraction_fail_on_error(
          f->fed_net_abstraction, MSG_TYPE_STOP_REQUEST_LENGTH, stop_request_buffer, &rti_mutex,
...
void handle_stop_request_reply(federate_info_t* fed) {
  ...
  read_from_net_abstraction_fail_on_error(
      fed->fed_net_abstraction, bytes_to_read, buffer_stop_time,
...
void handle_address_query(uint16_t fed_id) {
  ...
  read_from_net_abstraction_fail_on_error(fed->fed_net_abstraction, sizeof(uint16_t), (unsigned char*)buffer,
  ...
  if (remote_fed->fed_net_abstraction == NULL) {
    ...
    server_port = get_server_port(remote_fed->fed_net_abstraction);
    ip_address = (uint32_t*)get_ip_addr(remote_fed->fed_net_abstraction);
    server_host_name = get_server_hostname(remote_fed->fed_net_abstraction);
  }
  ...
  write_to_net_abstraction_fail_on_error(fed->fed_net_abstraction, 1 + sizeof(int32_t), (unsigned char*)buffer,
                                         &rti_mutex,
                                         "Failed to write port number to network abstraction of federate %d.", fed_id);

  write_to_net_abstraction_fail_on_error(fed->fed_net_abstraction, sizeof(uint32_t), (unsigned char*)ip_address,
                                         &rti_mutex,
...
void handle_address_ad(uint16_t federate_id) {
  ...
  read_from_net_abstraction_fail_on_error(fed->fed_net_abstraction, sizeof(int32_t), (unsigned char*)buffer,
  ...
  LF_MUTEX_LOCK(&rti_mutex);
  set_server_port(fed->fed_net_abstraction, server_port);
  LF_MUTEX_UNLOCK(&rti_mutex);
...
void handle_timestamp(federate_info_t* my_fed) {
  ...
  read_from_net_abstraction_fail_on_error(my_fed->fed_net_abstraction, sizeof(int64_t), (unsigned char*)&buffer,
  ...
  if (write_to_net_abstraction(my_fed->fed_net_abstraction, MSG_TYPE_TIMESTAMP_LENGTH, start_time_buffer)) {
...
void send_physical_clock(unsigned char message_type, federate_info_t* fed, bool use_UDP) {
  ...
  if (use_UDP) {
    ...
    // Send using network abstraction.
    LF_PRINT_DEBUG("Clock sync: RTI sending message type %u.", buffer[0]);
    LF_MUTEX_LOCK(&rti_mutex);
    write_to_net_abstraction_fail_on_error(fed->fed_net_abstraction, 1 + sizeof(int64_t), buffer, &rti_mutex,
...
static void handle_federate_failed(federate_info_t* my_fed) {
  ...
  shutdown_net_abstraction(my_fed->fed_net_abstraction, false);
 ...
 * function thus first performs a shutdown on the network abstraction, which sends an EOF. It then
 * waits for the remote network abstraction to be closed before closing the network abstraction itself.
 *
...
static void handle_federate_resign(federate_info_t* my_fed) {
  ...
  shutdown_net_abstraction(my_fed->fed_net_abstraction, true);
...
void* federate_info_thread_TCP(void* fed) {
  ...
  while (my_fed->enclave.state != NOT_CONNECTED) {
    ...
    int read_failed = read_from_net_abstraction(my_fed->fed_net_abstraction, 1, buffer);
    if (read_failed) {
      // network abstraction is closed
      lf_print_error("RTI: network abstraction to federate %d is closed. Exiting the thread.", my_fed->enclave.id);
      my_fed->enclave.state = NOT_CONNECTED;
      // Nothing more to do. Close the network abstraction and exit.
...
void send_reject(net_abstraction_t net_abs, unsigned char error_code) {
  ...
  if (write_to_net_abstraction(net_abs, 2, response)) {
    lf_print_warning("RTI failed to write MSG_TYPE_REJECT message on the network abstraction.");
  }
  // Close the network abstraction without reading until EOF.
  shutdown_net_abstraction(net_abs, false);
  LF_MUTEX_UNLOCK(&rti_mutex);
}
 ...
 * @param fed_net_abstraction Pointer to the network abstraction on which to listen.
 * @return The federate ID for success or -1 for failure.
 */
static int32_t receive_and_check_fed_id_message(net_abstraction_t fed_net_abstraction) {
  ...
  if (read_from_net_abstraction_close_on_error(fed_net_abstraction, length, buffer)) {
    lf_print_error("RTI failed to read from accepted network abstraction.");
    return -1;
  }
  ...
  if (buffer[0] != MSG_TYPE_FED_IDS) {
    ...
    if (buffer[0] == MSG_TYPE_P2P_SENDING_FED_ID || buffer[0] == MSG_TYPE_P2P_TAGGED_MESSAGE) {
      ...
      send_reject(fed_net_abstraction, WRONG_SERVER);
    } else if (buffer[0] == MSG_TYPE_FED_NONCE) {
      send_reject(fed_net_abstraction, RTI_NOT_EXECUTED_WITH_AUTH);
      lf_print_error("RTI not executed with HMAC authentication option using -a or --auth.");
    } else {
      send_reject(fed_net_abstraction, UNEXPECTED_MESSAGE);
    }
    ...
    if (read_from_net_abstraction_close_on_error(fed_net_abstraction, federation_id_length,
    ...
    if (strncmp(rti_remote->federation_id, federation_id_received, federation_id_length) != 0) {
      ...
      send_reject(fed_net_abstraction, FEDERATION_ID_DOES_NOT_MATCH);
      return -1;
    } else {
      if (fed_id >= rti_remote->base.number_of_scheduling_nodes) {
        ...
        send_reject(fed_net_abstraction, FEDERATE_ID_OUT_OF_RANGE);
        return -1;
      } else {
        if ((rti_remote->base.scheduling_nodes[fed_id])->state != NOT_CONNECTED) {
          ...
          send_reject(fed_net_abstraction, FEDERATE_ID_IN_USE);
          return -1;
        }
      }
    }
  ...
  fed->fed_net_abstraction = fed_net_abstraction;
  ...
  LF_MUTEX_LOCK(&rti_mutex);
  if (write_to_net_abstraction_close_on_error(fed->fed_net_abstraction, 1, &ack_message)) {
    LF_MUTEX_UNLOCK(&rti_mutex);
 ...
 */
static int receive_connection_information(net_abstraction_t fed_net_abstraction, uint16_t fed_id) {
  ...
  read_from_net_abstraction_fail_on_error(
      fed_net_abstraction, MSG_TYPE_NEIGHBOR_STRUCTURE_HEADER_SIZE, connection_info_header,
      "RTI failed to read MSG_TYPE_NEIGHBOR_STRUCTURE message header from federate %d.", fed_id);
  ...
  if (connection_info_header[0] != MSG_TYPE_NEIGHBOR_STRUCTURE) {
    ...
    if (connections_info_body_size > 0) {
      ...
      read_from_net_abstraction_fail_on_error(
          fed_net_abstraction, connections_info_body_size, connections_info_body,
          "RTI failed to read MSG_TYPE_NEIGHBOR_STRUCTURE message body from federate %d.", fed_id);
 ...
 * @param fed_net_abstraction The network abstraction on which to listen.
 * @param fed_id The federate ID.
 * @return 1 for success, 0 for failure.
 */
static int receive_udp_message_and_set_up_clock_sync(net_abstraction_t fed_net_abstraction, uint16_t fed_id) {
  ...
  read_from_net_abstraction_fail_on_error(fed_net_abstraction, 1 + sizeof(uint16_t), response,
    ...
    if (rti_remote->clock_sync_global_status >= clock_sync_init) {
      ...
      if (federate_UDP_port_number != UINT16_MAX) {
        ...
        for (int i = 0; i < rti_remote->clock_sync_exchanges_per_interval; i++) {
          ...
          size_t message_size = 1 + sizeof(uint16_t);
          unsigned char buffer[message_size];
          read_from_net_abstraction_fail_on_error(fed_net_abstraction, message_size, buffer,
                                                  "network abstraction to federate %d unexpectedly closed.", fed_id);
          if (buffer[0] == MSG_TYPE_CLOCK_SYNC_T3) {
      ...
      if (rti_remote->clock_sync_global_status >= clock_sync_on) {
        ...
        if (federate_UDP_port_number > 0) {
          ...
          fed->UDP_addr.sin_addr = *get_ip_addr(fed_net_abstraction);
        }
      } else {
 ...
 *
 * @param fed_net_abstraction network abstraction for the incoming federate tryting to authenticate.
 * @return True if authentication is successful and false otherwise.
 */
static bool authenticate_federate(net_abstraction_t fed_net_abstraction) {
  ...
  read_from_net_abstraction_fail_on_error(fed_net_abstraction, 1 + fed_id_length + NONCE_LENGTH, buffer,
  ...
  if (write_to_net_abstraction(fed_net_abstraction, 1 + NONCE_LENGTH + hmac_length, sender)) {
    lf_print_error("Failed to send nonce to federate.");
  }

  // Wait for MSG_TYPE_FED_RESPONSE
  unsigned char received[1 + hmac_length];
  read_from_net_abstraction_fail_on_error(fed_net_abstraction, 1 + hmac_length, received,
  ...
  if (memcmp(&received[1], rti_tag, hmac_length) != 0) {
    ...
    send_reject(fed_net_abstraction, HMAC_DOES_NOT_MATCH);
    return false;
  } else {
...
void lf_connect_to_federates(net_abstraction_t rti_net_abstraction) {
  for (int i = 0; i < rti_remote->base.number_of_scheduling_nodes; i++) {
    net_abstraction_t fed_net_abstraction = accept_net_abstraction(rti_net_abstraction, NULL);
    if (fed_net_abstraction == NULL) {
...
// Wait for the first message from the federate when RTI -a option is on.
    ...
    if (rti_remote->authentication_enabled) {
      if (!authenticate_federate(fed_net_abstraction)) {
        lf_print_warning("RTI failed to authenticate the incoming federate.");
        // Close the network abstraction without reading until EOF.
        shutdown_net_abstraction(fed_net_abstraction, false);
    ...
    int32_t fed_id = receive_and_check_fed_id_message(fed_net_abstraction);
    if (fed_id >= 0 && receive_connection_information(fed_net_abstraction, (uint16_t)fed_id) &&
        receive_udp_message_and_set_up_clock_sync(fed_net_abstraction, (uint16_t)fed_id)) {
...
void* respond_to_erroneous_connections(void* nothing) {
  ...
  while (true) {
    ...
    // or shutdown_net_abstraction(rti->rti_net_abstraction) is called.
    net_abstraction_t fed_net_abstraction = accept_net_abstraction(rti_remote->rti_net_abstraction, NULL);
    if (fed_net_abstraction == NULL) {
      return NULL;
    }
    ...
    if (write_to_net_abstraction(fed_net_abstraction, 2, response)) {
...
int start_rti_server() {
  ...
  rti_remote->rti_net_abstraction = initialize_net_abstraction();
  // Set the user specified port to the network abstraction.
  set_my_port(rti_remote->rti_net_abstraction, rti_remote->user_specified_port);
  // Create the server
  if (create_server(rti_remote->rti_net_abstraction, true)) {
...
void wait_for_federates() {
  ...
  lf_connect_to_federates(rti_remote->rti_net_abstraction);
  ...
  shutdown_net_abstraction(rti_remote->rti_net_abstraction, false);
