...
void initialize_local_rti(environment_t* envs, int num_envs) {
  ...
  rti_local->base.scheduling_nodes =
      (scheduling_node_t**)calloc(num_envs, sizeof(scheduling_node_t*));
  for (int i = 0; i < num_envs; i++) {
    enclave_info_t* enclave_info =
        (enclave_info_t*)malloc(sizeof(enclave_info_t));
    ...
    enclave_info->base.num_downstream =
        _lf_get_downstream_of(i, &enclave_info->base.downstream);
    enclave_info->base.num_upstream =
        _lf_get_upstream_of(i, &enclave_info->base.upstream);
...
void initialize_enclave_info(enclave_info_t* enclave, int idx,
  ...
  LF_ASSERT(lf_cond_init(&enclave->next_event_condition, &rti_mutex) == 0,
            "Could not create cond var");
}

tag_t rti_next_event_tag_locked(enclave_info_t* e, tag_t next_event_tag) {
  LF_PRINT_LOG("RTI: enclave %u sends NET of " PRINTF_TAG " ", e->base.id,
  ...
  tracepoint_federate_to_rti(e->env->trace, send_NET, e->base.id,
                             &next_event_tag);
  // First, update the enclave data structure to record this next_event_tag,
  // and notify any downstream scheduling_nodes, and unblock them if
  // appropriate.
  ...
  if (lf_tag_compare(e->base.last_granted, next_event_tag) >= 0) {
    LF_PRINT_LOG("RTI: enclave %u has already been granted a TAG to" PRINTF_TAG
                 ". Returning with a TAG to" PRINTF_TAG " ",
                 e->base.id, e->base.last_granted.time - lf_time_start(),
                 e->base.last_granted.microstep,
                 next_event_tag.time - lf_time_start(),
                 next_event_tag.microstep);
    tracepoint_federate_from_rti(e->env->trace, receive_TAG, e->base.id,
                                 &next_event_tag);
    // Release RTI mutex and re-enter the critical section of the source enclave
    // before returning.
    LF_ASSERT(lf_mutex_unlock(rti_local->base.mutex) == 0,
              "Could not unlock mutex");
    LF_ASSERT(lf_mutex_lock(&e->env->mutex) == 0, "Could not lock mutex");
    return next_event_tag;
  }

  // If this enclave has no upstream, then we give a TAG till forever straight
  // away.
  if (e->base.num_upstream == 0) {
    LF_PRINT_LOG("RTI: enclave %u has no upstream. Giving it a to the NET",
                 e->base.id);
  ...
  while (true) {
    ...
    LF_PRINT_LOG("RTI: enclave %u sleeps waiting for TAG to" PRINTF_TAG " ",
                 e->base.id, e->base.next_event.time - lf_time_start(),
                 e->base.next_event.microstep);
    LF_ASSERT(lf_cond_wait(&e->next_event_condition) == 0,
              "Could not wait for cond var");
  }

  // At this point we have gotten a new TAG.
  LF_PRINT_LOG("RTI: enclave %u returns with TAG to" PRINTF_TAG " ", e->base.id,
               e->base.next_event.time - lf_time_start(),
               e->base.next_event.microstep);
  tracepoint_federate_from_rti(e->env->trace, receive_TAG, e->base.id,
                               &result.tag);
  // Release RTI mutex and re-enter the critical section of the source enclave.
...
void rti_logical_tag_complete_locked(enclave_info_t* enclave, tag_t completed) {
  ...
  LF_ASSERT(lf_mutex_unlock(&enclave->env->mutex) == 0,
            "Could not unlock mutex");
  tracepoint_federate_to_rti(enclave->env->trace, send_LTC, enclave->base.id,
                             &completed);
...
void rti_update_other_net_locked(enclave_info_t* src, enclave_info_t* target,
  ...
  // acquire the RTI mutex. This means that we cannot block within this
  // function.
  LF_ASSERT(lf_mutex_lock(rti_local->base.mutex) == 0, "Could not lock mutex");
  tracepoint_federate_to_federate(src->env->trace, send_TAGGED_MSG,
                                  src->base.id, target->base.id, &net);
...
void notify_tag_advance_grant(scheduling_node_t* e, tag_t tag) {
  ...
  e->last_granted = tag;
  // TODO: Here we can consider adding a flag to the RTI struct and only signal
  // the cond var if we have sleeping enclaves.
  LF_ASSERT(lf_cond_signal(&((enclave_info_t*)e)->next_event_condition) == 0,
            "Could not signal cond var");
}
...
void notify_provisional_tag_advance_grant(scheduling_node_t* e, tag_t tag) {
  LF_PRINT_LOG("RTI: enclave %u callback with PTAG " PRINTF_TAG " ", e->id,
