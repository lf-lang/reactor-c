 ...
 * @brief Declarations for runtime infrastructure (RTI) for distributed Lingua
 * Franca programs. This file extends enclave.h with RTI features that are
 * specific to federations and are not used by scheduling enclaves.
 */
...
typedef struct federate_info_t {
  ...
  bool requested_stop; // Indicates that the federate has requested stop or has
                       // replied to a request for stop from the RTI. Used to
                       // prevent double-counting a federate when handling
                       // lf_request_stop().
  lf_thread_t thread_id; // The ID of the thread handling communication with
                         // this federate.
  ...
  bool clock_synchronization_enabled; // Indicates the status of clock
                                      // synchronization for this federate.
                                      // Enabled by default.
  in_transit_message_record_q_t*
      in_transit_message_tags; // Record of in-transit messages to this federate
                               // that are not yet processed. This record is
                               // ordered based on the time value of each
                               // message for a more efficient access.
  ...
  int32_t server_port; // port number of the socket server of the federate
                       // if it has any incoming direct connections from other
                       // federates. The port number will be -1 if there is no
                       // server or if the RTI has not been informed of the port
                       // number.
  struct in_addr server_ip_addr; // Information about the IP address of the
                                 // socket server of the federate.
} federate_info_t;
 ...
 * except that scheduling_nodes attribute here is of type `federate_info_t**`,
 * while it is of type `scheduling_node_t**` in `rti_common_t`.
 *     // **************** IMPORTANT!!! ********************
...
/**
 * Enter a critical section where logical time and the event queue are
 * guaranteed to not change unless they are changed within the critical section.
 ...
 *
 * It will update the recorded next event tag of federate `federate_id` to the
 * minimum of `next_event_tag` and the minimum tag of in-transit messages (if
 * any) to the federate.
 *
...
/**
 * Handle a timed message being received from a federate by the RTI to relay to
 * another federate.
 *
 * This function assumes the caller does not hold the mutex.
 *
 ...
 * This function reads the body of a MSG_TYPE_ADDRESS_QUERY (@see net_common.h)
 * message which is the requested destination federate ID and replies with the
 * stored port value for the socket server of that federate. The port values are
 * initialized to -1. If no MSG_TYPE_ADDRESS_ADVERTISEMENT message has been
 * received from the destination federate, the RTI will simply reply with -1 for
 * the port. The sending federate is responsible for checking back with the RTI
 * after a period of time. @see connect_to_federate() in federate.c. *
 * @param fed_id The federate sending a MSG_TYPE_ADDRESS_QUERY message.
 */
void handle_address_query(uint16_t fed_id);

/**
 * Handle address advertisement messages (@see MSG_TYPE_ADDRESS_ADVERTISEMENT in
 * net_common.h). The federate is expected to send its server port number as the
 * next byte. The RTI will keep a record of this number in the .server_port
 ...
 * @param socket_type The RTI's socket type used for the communication (TCP or
 * UDP)
 */
 ...
 * and then iterates over the federates, performing a complete clock
 * synchronization interaction with each federate before proceeding to the next
 * federate. The interaction starts with this RTI sending a snapshot of its
 * physical clock to the federate (message T1). It then waits for a reply and
 * then sends another snapshot of its physical clock (message T4).  It then
 * follows that T4 message with a coded probe message that the federate can use
 * to discard the session if the network is congested.
 */
 ...
 * Initial clock synchronization is performed only if the MSG_TYPE_UDP_PORT
 * message payload is not UINT16_MAX. If it is also not 0, then this function
 * sets up to perform runtime clock synchronization using the UDP port number
