...
//        'federate', or 'enlcave/federate'? Currently its is
//        'enclave/federate'.
// FIXME: Should scheduling_nodes tracing use the same mechanism as federates?
//        It needs to account a federate having itself a number of
//        scheduling_nodes. Currently, all calls to tracepoint_from_federate()
//        and tracepoint_to_federate() are in rti_lib.c
...
void _logical_tag_complete(scheduling_node_t* enclave, tag_t completed) {
  ...
  LF_PRINT_LOG("RTI received from federate/enclave %d the Logical Tag Complete "
               "(LTC) " PRINTF_TAG ".",
               enclave->id, enclave->completed.time - start_time,
               enclave->completed.microstep);

  // Check downstream scheduling_nodes to see whether they should now be granted
  // a TAG.
  for (int i = 0; i < enclave->num_downstream; i++) {
    scheduling_node_t* downstream =
        rti_common->scheduling_nodes[enclave->downstream[i]];
    // Notify downstream enclave if appropriate.
    notify_advance_grant_if_safe(downstream);
    bool* visited = (bool*)calloc(rti_common->number_of_scheduling_nodes,
...
tag_advance_grant_t tag_advance_grant_if_safe(scheduling_node_t* e) {
  ...
  for (int j = 0; j < e->num_upstream; j++) {
    ...
    tag_t candidate =
        lf_delay_strict(upstream->completed, e->upstream_delay[j]);
  ...
  LF_PRINT_LOG(
      "RTI: Minimum upstream LTC for federate/enclave %d is " PRINTF_TAG
      "(adjusted by after delay).",
      e->id, min_upstream_completed.time - start_time,
      min_upstream_completed.microstep);
  if (lf_tag_compare(min_upstream_completed, e->last_granted) > 0 &&
      lf_tag_compare(min_upstream_completed, e->next_event) >=
          0 // The enclave has to advance its tag
  ) {
  ...
  // The tag of the earliest possible incoming message from a zero-delay
  // connection. Delayed connections are not guarded from STP violations by the
  // MLAA; this property is acceptable because delayed connections impose no
  // deadlock risk and in some cases (startup) this property is necessary to
  // avoid deadlocks. However, it requires some special care here when
  // potentially sending a PTAG because we must not send a PTAG for a tag at
  // which data may still be received over nonzero-delay connections.
  tag_t t_d_zero_delay = FOREVER_TAG;
  LF_PRINT_DEBUG("NOTE: FOREVER is displayed as " PRINTF_TAG
                 " and NEVER as " PRINTF_TAG,
  ...
  for (int j = 0; j < e->num_upstream; j++) {
    ...
    tag_t upstream_next_event =
        transitive_next_event(upstream, upstream->next_event, visited);

    LF_PRINT_DEBUG("RTI: Earliest next event upstream of fed/encl %d at "
                   "fed/encl %d has tag " PRINTF_TAG ".",
                   e->id, upstream->id, upstream_next_event.time - start_time,
    ...
    // whereas one microstep delay is encoded as 0LL.
    tag_t candidate =
        lf_delay_strict(upstream_next_event, e->upstream_delay[j]);
  ...
  free(visited);
  tag_t t_d = (lf_tag_compare(t_d_zero_delay, t_d_nonzero_delay) < 0)
                  ? t_d_zero_delay
                  : t_d_nonzero_delay;
  ...
  if (lf_tag_compare(t_d, e->next_event) > 0 // The enclave has something to do.
      && lf_tag_compare(t_d, e->last_provisionally_granted) >=
             0 // The grant is not redundant
               // (equal is important to override any previous
               // PTAGs).
      && lf_tag_compare(t_d, e->last_granted) > 0 // The grant is not redundant.
  ) {
    // All upstream scheduling_nodes have events with a larger tag than fed, so
    // it is safe to send a TAG.
    LF_PRINT_LOG(
        "Earliest upstream message time for fed/encl %d is " PRINTF_TAG
        "(adjusted by after delay). Granting tag advance for " PRINTF_TAG,
        e->id, t_d.time - lf_time_start(), t_d.microstep,
        e->next_event.time - lf_time_start(), e->next_event.microstep);
    result.tag = e->next_event;
  } else if (lf_tag_compare(t_d_zero_delay, e->next_event) ==
                 0 // The enclave has something to do.
             && lf_tag_compare(t_d_zero_delay, t_d_nonzero_delay) <
                    0 // The statuses of nonzero-delay connections are known at
                      // tag t_d_zero_delay
             && lf_tag_compare(t_d_zero_delay, e->last_provisionally_granted) >
                    0 // The grant is not redundant.
             && lf_tag_compare(t_d_zero_delay, e->last_granted) >
                    0 // The grant is not redundant.
  ) {
    // Some upstream scheduling_nodes has an event that has the same tag as
    // fed's next event, so we can only provisionally grant a TAG (via a PTAG).
    LF_PRINT_LOG(
        "Earliest upstream message time for fed/encl %d is " PRINTF_TAG
        " (adjusted by after delay). Granting provisional tag advance.",
        e->id, t_d_zero_delay.time - start_time, t_d_zero_delay.microstep);
...
void notify_downstream_advance_grant_if_safe(scheduling_node_t* e,
  ...
  for (int i = 0; i < e->num_downstream; i++) {
    scheduling_node_t* downstream =
        rti_common->scheduling_nodes[e->downstream[i]];
...
void update_scheduling_node_next_event_tag_locked(scheduling_node_t* e,
  ...
  LF_PRINT_DEBUG("RTI: Updated the recorded next event tag for "
                 "federate/enclave %d to " PRINTF_TAG,
                 e->id, next_event_tag.time - lf_time_start(),
  ...
  // Check downstream scheduling_nodes to see whether they should now be granted
  // a TAG. To handle cycles, need to create a boolean array to keep track of
  // which upstream scheduling_nodes have been visited.
  bool* visited = (bool*)calloc(rti_common->number_of_scheduling_nodes,
                                sizeof(bool)); // Initializes to 0.
...
tag_t transitive_next_event(scheduling_node_t* e, tag_t candidate,
  ...
  for (int i = 0; i < e->num_upstream; i++) {
    ...
    // If the adjusted event time is less than the result so far, update the
    // result.
