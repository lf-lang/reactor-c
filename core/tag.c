...
//////////////// Global variables not declared in tag.h (must be declared extern
/// if used elsewhere):
...
instant_t _lf_physical_time() {
  ...
  LF_PRINT_DEBUG("Physical time: " PRINTF_TIME ". Elapsed: " PRINTF_TIME
                 ". Offset: " PRINTF_TIME,
...
int lf_tag_compare(tag_t tag1, tag_t tag2) {
  if (tag1.time < tag2.time) {
    ...
    LF_PRINT_DEBUG(PRINTF_TIME " and microstep < " PRINTF_TIME, tag1.time,
                   tag2.time);
    return -1;
  } else if (tag1.microstep > tag2.microstep) {
    return 1;
...
tag_t lf_delay_strict(tag_t tag, interval_t interval) {
  ...
  if (interval != 0 && interval != NEVER && interval != FOREVER &&
    ...
    LF_PRINT_DEBUG("interval=%lld, result time=%lld", (long long)interval,
...
instant_t lf_time_physical(void) { return _lf_physical_time(); }
