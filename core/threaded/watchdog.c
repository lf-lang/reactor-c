 ...
 * For any reactor with one or more watchdogs, the self struct should have a
 * non-NULL `reactor_mutex` field which points to an instance of `lf_mutex_t`.
...
void lf_watchdog_start(watchdog_t* watchdog, interval_t additional_timeout) {
  ...
  watchdog->expiration = base->environment->current_tag.time +
                         watchdog->min_expiration + additional_timeout;
...
void lf_watchdog_stop(watchdog_t* watchdog) { watchdog->expiration = NEVER; }
